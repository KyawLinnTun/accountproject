...
public class CashBookListActionBean extends BaseBean {
	...
	private StreamedContent getDownloadValue(List<CashBookDTO> dtoList) {
		try {
			...
			String currencyCode = criteria.getCurrencyType().equals(CurrencyType.HOMECURRENCY) ? "MMK" : criteria.getCurrency().getCurrencyCode();
			...
			for (CashBookDTO cashBook : dtoList) {
				...
				paramMap.put("currencyName", currencyCode);
