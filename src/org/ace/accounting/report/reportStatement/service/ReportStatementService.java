package org.ace.accounting.report.reportStatement.service;
...
import org.ace.accounting.web.common.FIType;
...
public class ReportStatementService extends BaseService implements IReportStatementService {
	...
	public List<ReportStatementDto> previewProcedure(boolean isObal, ReportType reportType, CurrencyType currencyType,
			Currency currency, Branch branch, Date reportDate, String formatType) throws Exception {
		List<ReportStatementDto> result = null;
		try {
			result = reportStatementDAO.previewProcedure(isObal, reportType, currencyType, currency, branch, reportDate,
					formatType);
	...
	public List<ReportStatementDto> liabilitiesAcodeCheck(LiabilitiesACDto plDto, LiabilitiesACDto taxDto,
			List<ReportStatementDto> dtoList) {
		try {
			...
			for (ReportStatementDto dto : dtoList) {
				if (departmentStatus) {
					if (plDto.getAcCode().equals(dto.getAcCode())) {
						if ((dto.getdCode() == null && plDto.getdCode() == null)
								|| (dto.getdCode() != null && dto.getdCode().equals(plDto.getdCode()))) {
							dto.setcBal(plDto.getcBal());
						}
					}
					if (taxDto.getAcCode().equals(dto.getAcCode())) {
						if ((dto.getdCode() == null && taxDto.getdCode() == null)
								|| (dto.getdCode() != null && dto.getdCode().equals(taxDto.getdCode()))) {
	...
	private List<ReportStatementDto> formulaProcess(List<ReportStatementDto> dtoList, ReportType reportType,
	...
	// {1-2}
	private ReportStatementDto calculateFormula(ColType coltype, List<ReportStatementDto> dtoList,
			ReportStatementDto targetDto, String formula) throws Exception {
						...
						if (nextType.equals(FIType.COMMADOT)) {
							...
							if (startLine > 0 && commaExist) {
								...
								if (coltype.equals(ColType.L)) {
									...
									targetDto = handleOperator(targetDto, rangeCbalTotal, rangeObalTotal, rangeAmtTotal,
											rangeBfamountTotal, rangeTotalTotal, null, prevOperator);
									...
									targetDto = handleOperator(targetDto, null, null, null, null, null, rangeRamtTotal,
											prevOperator);
									...
									targetDto = handleOperator(targetDto, result, null, null, null, null, result,
											prevOperator);
								}
								numString = "";
								prevOperator = null;
								...
								// - 1)));
//								ReportStatementDto prevLineDto = getAmountOfLine(dtoList, startLine);
								ReportStatementDto lineDto = getAmountOfLine(dtoList, Integer.parseInt(numString));
								if (coltype.equals(ColType.L)) {
//									if (null != prevLineDto) {
//										targetDto = handleOperator(targetDto, prevLineDto.getcBal(),
//												prevLineDto.getoBal(), prevLineDto.getAmt(), prevLineDto.getBfAmount(),
//												prevLineDto.getTotal(), null, "+");
//									}
//
//									targetDto = handleOperator(targetDto, lineDto.getcBal(), lineDto.getoBal(),
//											lineDto.getAmt(), lineDto.getBfAmount(), lineDto.getTotal(), null,
//											prevOperator);
//								} else if (coltype.equals(ColType.R)) {
//									if (null != prevLineDto) {
//										targetDto = handleOperator(targetDto, null, null, null, null, null,
//												prevLineDto.getrAmt(), "+");
//									}
//
//									targetDto = handleOperator(targetDto, null, null, null, null, null,
...
//								} else if (coltype.equals(ColType.B)) {
//									if (null != prevLineDto) {
//										targetDto = handleOperator(targetDto, prevLineDto.getcBal(), null, null, null,
//												null, prevLineDto.getrAmt(), "+");
//									}
//
//									targetDto = handleOperator(targetDto, lineDto.getcBal(), null, null, null, null,
//											lineDto.getrAmt(), prevOperator);
//								}
								...
								targetDto = handleOperator(targetDto, lineDto.getcBal(), lineDto.getoBal(),lineDto.getAmt(), lineDto.getBfAmount(), lineDto.getTotal(), null,
									...
									targetDto = handleOperator(targetDto, null, null, null, null, null,lineDto.getrAmt(), prevOperator);
								} else if (coltype.equals(ColType.B)) {
						...
						if (prevOperator != null) {
							if (coltype.equals(ColType.L)) {
								targetDto = handleOperator(targetDto, new BigDecimal(numString),
										new BigDecimal(numString), new BigDecimal(numString), new BigDecimal(numString),
										new BigDecimal(numString), null, prevOperator);
							} else if (coltype.equals(ColType.R)) {
								targetDto = handleOperator(targetDto, null, null, null, null, null,
										new BigDecimal(numString), prevOperator);
							} else if (coltype.equals(ColType.B)) {
								targetDto = handleOperator(targetDto, new BigDecimal(numString), null, null, null, null,
										new BigDecimal(numString), prevOperator);
...
//				startLine = Integer.parseInt(numString);
	...
	private ReportStatementDto handleOperator(ReportStatementDto resultDto, BigDecimal cbal, BigDecimal obal,
			BigDecimal amt, BigDecimal bfamt, BigDecimal total, BigDecimal ramt, String prevOperator) throws Exception {
		if (prevOperator != null) {
